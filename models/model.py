...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if prompt_len > 0:
            self.prompt_pos_embedding = nn.Parameter(
                scale * torch.zeros(prompt_len, width)
            )
    ...
    def forward(self, x: torch.Tensor, ind_prompt: torch.Tensor = None):
        ...
        if self.prompt_len > 0 and ind_prompt != None:
            tmp_ind_prompt = ind_prompt + torch.zeros(
            ...
            x = x + torch.cat(
                [self.positional_embedding.to(x.dtype), self.prompt_pos_embedding],
                dim=0,
            )
        else:
            x = torch.cat(
...
def clip_img_preprocessing(X, device):
    # put X to cpu to avoid memory error
    # X = X.cpu()
    img_size = 224
...
def multiGPU_CLIP_classwise(clip_model, images, text_tokens, prompt_token=None):
    ...
    img_embed = img_embed.unsqueeze(0)  # (1, bs, 512)
    img_embed = img_embed.expand(ncaps, -1, -1)  # (ncaps, bs, 512)

    logits_per_image = torch.bmm(
        img_embed, scale_text_embed.permute(0, 2, 1)
    )  # (ncaps, bs, 512) @ (ncaps, 512, n_class) -> (ncaps, bs, n_class)
    logits_per_text = torch.bmm(
        scale_text_embed, img_embed.permute(0, 2, 1)
